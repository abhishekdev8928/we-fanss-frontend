...
const PositionsList = () => {
  ...
  const fetchPositions = async () => {
    try {
      const result = await getPositionsByCelebrity(celebrityId);
      setPositions(result.msg || []);
    } catch (error) {
  ...
  const handleChange = async (currentStatus, id) => {
    ...
    try {
      const res_data = await updatePositionsStatus(id, newStatus);
      ...
      toast.success("Positions status updated successfully");
      fetchPositions();
    } catch (error) {
  ...
  const handleConfirmDelete = async () => {
    if (!deleteId) return;
    try {
      const result = await deletePositions(deleteId);
      if (result.status) {
        toast.success("Positions deleted successfully!");
        setPositions((prev) => prev.filter((m) => m._id !== deleteId));
        setModalOpen2(false);
      } else {
        toast.error(result.msg || "Failed to delete Positions.");
      }
    } catch (error) {
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div className="d-flex gap-2">
            <Link
              to={`/dashboard/update-positions/${row.original._id}`}
  ...
  return (
    <Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="Positions" breadcrumbItems={breadcrumbItems} />
          <Card>
            <CardBody>
             <div className="d-flex justify-content-between align-items-center mb-3">
                           ...
                           <div className="d-flex gap-2">
                             <Link to={`/dashboard/add-positions/${id}`} className="btn btn-primary">
                               + Add Positions
                             </Link>
                             <Button
                               color="secondary"
        ...
        <Modal isOpen={modalOpen2} toggle={handleClose}>
          <ModalBody className="mt-3">
            <h4 className="p-3 text-center">
              Do you really want to delete this Positions?
            </h4>
