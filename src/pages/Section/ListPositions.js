...
const PositionsList = () => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (result?.status) {
        ...
        setPositions((prev) => prev?.filter((row) => row?._id !== deleteId));
        setDeleteModalOpen(false);
        setDeleteId(null);
      } else {
        toast.error(result?.msg || "Failed to delete position");
      }
    } catch (error) {
      console.error("Error:", error);
  ...
  // ✅ Handle Add
  const handleAdd = () => {
    navigate(`/dashboard/add-positions/${id}`);
  };
  ...
  }, [celebrityId, fetchPositions, fetchCelebrityName]);
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <div className="d-flex gap-2">
          <Link to={`/dashboard/update-positions/${row?.original?._id}`}>
            <Button
              style={{
