...
const MovievList = () => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (result?.status) {
        ...
        setMovies((prev) => prev?.filter((row) => row?._id !== deleteId));
  ...
  // ✅ Handle Add
  const handleAdd = () => {
    navigate(`/dashboard/add-movie/${id}`);
  };
  ...
  }, [celebrityId, fetchMovies, fetchCelebrityName]);
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <div className="d-flex gap-2">
          <Link to={`/dashboard/update-movie/${row?.original?._id}`}>
            <Button
              style={{
